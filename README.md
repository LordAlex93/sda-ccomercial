# Situación de Aprendizaje - Construye tu centro comercial

Situación de aprendizaje desarrollada con eXeLearning sobre la proporcionalidad y el porcentaje, contextualizada en un centro comercial para 2º de ESO.

## Contenido del repositorio

- `docs/`: sitio web exportado desde eXeLearning para su publicación en GitHub Pages.

## Publicación

Este proyecto está preparado para publicarse con GitHub Pages desde la carpeta `docs/`.

## Autoría

Alejandro Muñoz Azaustre

Carolina Sánchez Rodríguez

## Licencia

Este trabajo está bajo la licencia **Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International (CC BY-NC-SA 4.0)**.
